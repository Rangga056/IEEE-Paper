...
\setcounter{topnumber}{1}      % Maximum 1 float at the top of a column
\setcounter{bottomnumber}{1}   % Maximum 1 float at the bottom of a column
\setcounter{totalnumber}{1}    % Maximum 1 float total per column
\setcounter{dbltopnumber}{1}   % Maximum 1 spanning float per page top
...
The Jakarta Composite Index (IHSG) is a critical indicator of Indonesia’s economy, increasingly sensitive to global dynamics such as fluctuations in the S\&P 500, commodities, and macroeconomic shifts, which introduce nonlinear effects \cite{b1}, \cite{b10}. Accurate forecasting of the IHSG is essential for investors and policymakers to manage risk and ensure economic stability.

Traditional econometric models often fail to capture the complex, non-stationary, and volatile nature of financial markets, limiting their ability to model long-range dependencies and interactions among various features. To address these limitations, we propose a novel parallel stacking ensemble that integrates a Long Short-Term Memory (LSTM) network and a TabNet model. This ensemble, aggregated by an ElasticNet meta-learner, provides superior accuracy compared to standalone models and serial hybrid architectures for forecasting the IHSG \cite{b21}. Additionally, we identify recent global volatility and lagged returns (S\&P 500, Gold) as key predictors of IHSG movements.


\par The main contributions of this paper are threefold. First, we propose a novel \textit{parallel stacking} framework that integrates an LSTM (for raw temporal patterns) with a TabNet model (for engineered tabular features). This approach contrasts with common serial hybrids (e.g., ARIMA-LSTM) where models are chained. Second, we demonstrate that this ensemble, aggregated by an ElasticNet meta-learner (chosen for its ability to handle correlated predictions and prevent overfitting), yields superior accuracy compared to standalone models and, significantly, to serial hybrid architectures for IHSG forecasting \cite{b21}. Third, we provide an interpretable analysis identifying recent global volatility and lagged returns (S\&P 500, Gold) as dominant predictors.

To overcome these constraints, we propose a stacking ensemble model combining a Long Short-Term Memory (LSTM) network with a TabNet architecture \cite{b2}, \cite{b3}. The LSTM component models temporal dependencies directly from historical price sequences \cite{b3}. In parallel, the TabNet model is trained on engineered temporal features (e.g., rolling means, trends). Predictions from both base models are then fed into an ElasticNet meta-learner. We specifically selected ElasticNet for its regularization properties (combining L1 and L2 penalties). An ElasticNet is highly effective at managing multicollinearity, a common issue when base model predictions are highly correlated, and preventing overfitting in the final aggregation step \cite{b4}. This research demonstrates that this parallel stacking approach significantly enhances predictive accuracy \cite{b20}.

\section{Literature Review and Research Gap}

\subsection{Current State of Financial Time-Series Forecasting}
Financial time-series forecasting has evolved significantly with the use of machine learning models to address the limitations of traditional econometrics. Key approaches include deep learning models, such as Long Short-Term Memory (LSTM) networks, which capture temporal dependencies in financial data \cite{b3}, \cite{b20}, and attention-based models like TabNet, which excel in feature selection for tabular data \cite{b7}, \cite{b11}. However, these models face challenges with high-dimensional feature spaces, limiting their effectiveness when used alone.

Hybrid models, such as ARIMA-CNN-LSTM combinations, and tree-based ensembles like Random Forest, have also been explored \cite{b5}, offering complementary strengths in financial forecasting. The challenge lies in effectively integrating these models without creating bottlenecks, especially in serial hybrid architectures.
...
LSTM networks are RNNs designed to capture long-range dependencies, addressing the vanishing gradient problem \cite{b20}. Their input, forget, and output gates regulate information flow, allowing the network to selectively remember patterns. Our LSTM is applied to the raw 90-timestep sequences to model raw temporal dynamics.

\subsection{TabNet}
TabNet is a deep architecture for tabular data using sequential attention to select features at each decision step \cite{b7}, \cite{b11}. This process mimics decision trees, offering interpretability without sacrificing performance. We train TabNet on engineered tabular features (rolling means, MACD, etc.) to model their nonlinear interactions \cite{b26}.

\subsection{Elastic Net Meta-Learner}
Predictions from base models are aggregated using an Elastic Net meta-learner \cite{b4}. It combines L1 (Lasso) and L2 (Ridge) regularization, making it effective when base model predictions are correlated. It prevents overfitting and adds minimal complexity.
...
The performance of all forecasting models was rigorously evaluated on the unseen test set using a suite of widely accepted metrics. The selection of these metrics ensures a comprehensive assessment of both prediction accuracy and practical utility for financial applications.
\begin{itemize}
  \item \textbf{Root Mean Squared Error (RMSE):} RMSE was chosen because it penalizes large errors more heavily than small ones. In finance, large prediction errors can lead to significant financial losses, making RMSE a critical indicator of model robustness against high-volatility events.
  \item \textbf{Mean Absolute Error (MAE):} Unlike RMSE, MAE treats all errors equally, providing a more direct and intuitive interpretation of the average error magnitude in the same units as the target variable (returns). It is less sensitive to outliers and complements RMSE by showing the model's average accuracy.
  \item \textbf{R-Squared (R²):} The inclusion of R-Squared (R2) quantifies the variance explained by the model regarding financial returns. High R2 values indicate that the model captures essential market trends rather than merely reflecting a static mean response, enhancing the reliability of forecasting results \cite{b35}, \cite{b38}. R-Squared is widely regarded as critical in regression analysis, providing essential insights into the model's explanatory power \cite{b35}, \cite{b37}.
  \item \textbf{Symmetric Mean Absolute Percentage Error (SMAPE):} SMAPE is useful for comparing model performance across different assets or time periods due to its scale-independent nature. SMAPE avoids the challenge of "infinite error" associated with conventional Mean Absolute Percentage Error (MAPE) when actual values are zero, ensuring a more reliable measure of performance \cite{b35}, \cite{b39}. Consequently, employing SMAPE allows for a more nuanced evaluation of model efficiency, especially when forecasting financial instruments with varying value scales \cite{b39}.
  \item \textbf{Paired t-test:} This test is used to compare the distributions of the prediction errors (specifically, the squared errors) between two models. It tests the null hypothesis that the difference in squared errors has a mean of zero, providing insight into whether one model's performance is statistically significantly different from another's.
  \item \textbf{Wilcoxon Signed-Rank Test:} As a non-parametric alternative to the t-test, the Wilcoxon test is used to compare the paired error distributions without assuming they follow a normal distribution. This makes it a more robust test if the error data is skewed.
  \item \textbf{Diebold-Mariano (DM) Test:} To further validate comparative accuracy, statistical tests such as the Diebold-Mariano (DM) test are employed. An econometric tool, the DM test assesses whether there is a statistically significant difference between the forecasting performances of two models. A low p-value (e.g., $<$ 0.05) from the DM test enables researchers to reject the null hypothesis, indicating that one model outperforms another \cite{b40}. Such rigorous statistical validation reinforces confidence in selecting superior predictive models based on demonstrated performance, as outlined in various forecasting analyses \cite{b40}, \cite{b33}.
\end{itemize}
